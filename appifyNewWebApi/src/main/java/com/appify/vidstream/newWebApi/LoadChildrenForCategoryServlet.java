...
public class LoadChildrenForCategoryServlet extends HttpServlet {
	...
	public LoadChildrenForCategoryServlet(@RequestParameters Provider<Map<String, String[]>> paramsProvider,
			...
			ExploreCategoryDataLoader exploreCategoryLoader) {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (entityCollection.getEntityType() == EntityType.CATEGORY) {
			...
			for (int i = 0; i < categories.size(); i++) {
				...
				CategoryResp categoryResp = converter
			...
			if (entityCollection.getEntities() != null && entityCollection.getEntities().size() > 0) {
				...
				for (int i = 0; i < currOrderedVideosList.size(); i++) {
					...
					orderAttributesApi[i] = VideoAttribute.getAttributeByDataName(currOrderedVideos.getName())
							.getApiName();
				}
				...
				videoResps = new VideoResp[endIndex - startIndex + 1];
